...
<div class="card">
    ...
    <?php endif; ?>
    <?php if($wave): ?>
    <div class="muted">Hạn đăng ký: Mã từ <?php echo e(\Carbon\Carbon::parse($wave->starts_at)->format('d/m/Y')); ?> / Hạn đăng ký <?php echo e(\Carbon\Carbon::parse($wave->ends_at)->format('d/m/Y')); ?>, Hạn chót: <?php echo e(\Carbon\Carbon::parse($wave->ends_at)->format('d/m/Y H:i')); ?></div>
    <?php endif; ?>
</div>

<div class="card">
    <h3 style="margin:0 0 12px 0;color:#1976d2;font-size:16px;">Đợt đăng ký đang mở</h3>
    <form method="GET" action="<?php echo e(route('student.offerings')); ?>" style="display:grid;gap:8px;grid-template-columns:repeat(6, 1fr);align-items:end;margin-bottom:16px;">
        <div>
            <label class="muted" style="display:block;margin-bottom:4px;">Tìm kiếm</label>
            <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Mã môn, tên môn, mã lớp" style="width:100%;padding:8px;border-radius:4px;border:1px solid #ddd;">
        </div>
        <div>
            <label class="muted" style="display:block;margin-bottom:4px;">Năm học</label>
            <input type="text" name="academic_year" value="<?php echo e($year); ?>" style="width:100%;padding:8px;border-radius:4px;border:1px solid #ddd;" placeholder="2024-2025">
        </div>
        <div>
            ...
            <select name="term" style="width:100%;padding:8px;border-radius:4px;border:1px solid #ddd;">
                <option value="HK1" <?php echo e($term=='HK1'?'selected':''); ?>>HK1</option>
                <option value="HK2" <?php echo e($term=='HK2'?'selected':''); ?>>HK2</option>
                <option value="HK3" <?php echo e($term=='HK3'?'selected':''); ?>>HK3</option>
            </select>
        </div>
        <div>
        ...
        <div>
            ...
            <select name="shift_id" style="width:100%;padding:8px;border-radius:4px;border:1px solid #ddd;">
                <option value="">-- Tất cả --</option>
                <?php if(isset($shifts)): ?>
                <?php $__currentLoopData = $shifts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sh): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($sh->id); ?>" <?php echo e(request('shift_id')==$sh->id?'selected':''); ?>>Tiết <?php echo e($sh->start_period); ?>-<?php echo e($sh->end_period); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                <?php endif; ?>
            </select>
        </div>
        <div>
            ...
            <select name="room_id" style="width:100%;padding:8px;border-radius:4px;border:1px solid #ddd;">
                ...
                <option value="<?php echo e($r->id); ?>" <?php echo e(request('room_id')==$r->id?'selected':''); ?>><?php echo e($r->code); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                <?php endif; ?>
            </select>
        </div>
        <div style="grid-column: 1 / -1; display:flex; align-items:center; gap:12px; justify-content:space-between;">
            <label class="muted" style="display:flex;align-items:center;gap:8px;">
                <input type="checkbox" name="only_available" value="1" <?php echo e(request('only_available') ? 'checked' : ''); ?>>
    ...
    <div class="grid" style="grid-template-columns: 2fr 1fr; gap:16px;margin-top:16px;">
        <div>
            ...
            <div class="courses-grid" style="display:flex;flex-direction:column;gap:16px;">
                ...
                <div class="course-card" data-course-id="<?php echo e($course->id); ?>" style="
                    background:white;
                    border-radius:12px;
                    ...
                    <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:12px;">
                        <div style="flex:1;">
                            <h3 style="margin:0 0 8px 0;font-size:18px;color:#1976d2;font-weight:600;">
                                <span class="course-code" style="background:#e3f2fd;padding:4px 12px;border-radius:6px;font-size:14px;margin-right:8px;"><?php echo e($course->code); ?></span>
                            ...
                            <div style="display:flex;gap:16px;align-items:center;flex-wrap:wrap;">
                                <span style="color:#666;font-size:14px;">
                                    ...
                                    <strong><?php echo e($course->credits); ?></strong> tín chỉ
                                </span>
                                <span style="color:#666;font-size:14px;">
                                    <i class="fas fa-university" style="color:#1976d2;margin-right:6px;"></i>
                                    <?php echo e($course->faculty?->name ?? 'N/A'); ?>

                                </span>
                                <span style="color:#666;font-size:14px;">
                                    <i class="fas fa-users" style="color:#1976d2;margin-right:6px;"></i>
                                    <strong><?php echo e($course->total_sections); ?></strong> lớp mở
                                </span>
                            </div>
                        </div>
                        <div style="display:flex;flex-direction:column;align-items:end;gap:8px;">
                            <?php if(in_array($course->id, $registeredCourseIds)): ?>
                            ...
                            <?php elseif($course->available_sections > 0): ?>
                            <span style="background:#d1ecf1;color:#0c5460;padding:8px 16px;border-radius:8px;font-weight:600;font-size:14px;white-space:nowrap;">
                                <i class="fas fa-door-open"></i> <?php echo e($course->available_sections); ?> lớp còn chỗ
                            </span>
                            <?php else: ?>
            ...
            <div style="margin-top:20px;"><?php echo e($courses->links()); ?></div>
        </div>
        ...
        <div>
            ...
            <div style="overflow-x:auto;">
                <table>
                    ...
                    <tbody>
                        <?php $__empty_1 = true; $__currentLoopData = $currentRegs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $r): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <?php ($cs = $r->classSection); ?>
                        <tr>
                            <td>
                                <div><strong><?php echo e($cs->course->code); ?></strong></div>
                                <div class="muted"><?php echo e($cs->course->name); ?></div>
                            </td>
                            <td>
                                <div><?php echo e($cs->shift->day_name ?? ('Thứ '.$cs->day_of_week)); ?></div>
                                <div class="muted">T<?php echo e($cs->shift->start_period); ?>-<?php echo e($cs->shift->end_period); ?></div>
                            </td>
                            <td><?php echo e($cs->course->credits); ?></td>
                            <td>
                                <form action="<?php echo e(route('student.cancel', $r)); ?>" method="POST" onsubmit="return confirm('Hủy lớp này?')">
                                    <?php echo csrf_field(); ?>
...
<?php echo $__env->make('student.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\quanlytkbieu\resources\views/student/registrations/offerings.blade.php ENDPATH**/ ?>