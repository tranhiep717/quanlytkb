...
<div class="card mb-3">
    <div class="card-body">
        <div class="row g-3 align-items-end">
            <form class="col-12 col-lg-6" method="GET" action="{{ route('lecturer.dashboard') }}">
                <div class="row g-2">
            ...
            <div class="col-12 col-lg-6 d-flex flex-wrap justify-content-lg-end align-items-center gap-2">
                <div class="btn-group" role="group" aria-label="View Mode">
                    <a class="btn btn-outline-secondary btn-sm {{ $view==='week' ? 'active' : '' }}" href="{{ route('lecturer.dashboard', ['academic_year'=>$academicYear,'term'=>$term,'view'=>'week','date'=>$baseDate]) }}">
                        <i class="fas fa-calendar-week me-1"></i>Tuần
                    </a>
                    <a class="btn btn-outline-secondary btn-sm {{ $view==='month' ? 'active' : '' }}" href="{{ route('lecturer.dashboard', ['academic_year'=>$academicYear,'term'=>$term,'view'=>'month','date'=>$baseDate]) }}">
                        <i class="fas fa-calendar-alt me-1"></i>Tháng
                    </a>
                    <a class="btn btn-outline-secondary btn-sm {{ $view==='list' ? 'active' : '' }}" href="{{ route('lecturer.dashboard', ['academic_year'=>$academicYear,'term'=>$term,'view'=>'list','date'=>$baseDate]) }}">
                        <i class="fas fa-list me-1"></i>Danh sách
                    </a>
                </div>

                <div class="btn-group" role="group" aria-label="Navigate">
                    <a class="btn btn-outline-secondary btn-sm" title="Trước" href="{{ route('lecturer.dashboard', ['academic_year'=>$academicYear,'term'=>$term,'view'=>$view,'date'=>$prevDate]) }}">
                    ...
                    <a class="btn btn-outline-secondary btn-sm" title="Sau" href="{{ route('lecturer.dashboard', ['academic_year'=>$academicYear,'term'=>$term,'view'=>$view,'date'=>$nextDate]) }}">
                        <i class="fas fa-chevron-right"></i>
                    </a>
                    <a class="btn btn-outline-primary btn-sm" href="{{ route('lecturer.dashboard', ['academic_year'=>$academicYear,'term'=>$term,'view'=>$view]) }}">Hôm nay</a>
                </div>

                <div class="btn-group" role="group" aria-label="Exports">
                    <a class="btn btn-outline-success btn-sm" href="{{ route('lecturer.timetable.exportCsv', ['academic_year'=>$academicYear,'term'=>$term]) }}">
                        <i class="fas fa-file-excel me-1"></i>Excel/CSV
                    </a>
                    <a class="btn btn-outline-danger btn-sm" target="_blank" href="{{ route('lecturer.timetable.print', ['academic_year'=>$academicYear,'term'=>$term]) }}">
                        <i class="fas fa-file-pdf me-1"></i>PDF
                    </a>
                    <a class="btn btn-outline-dark btn-sm" href="{{ route('lecturer.timetable.ics', ['academic_year'=>$academicYear,'term'=>$term]) }}">
                    ...
                    @for($slot = 0; $slot < max(1, $maxSlots); $slot++)
                        ...
                        <td class="p-2" style="vertical-align: top;">
                            ...
                            <div class="class-box p-3" style="background: linear-gradient(135deg, #e3f2fd, #bbdefb); border-left: 4px solid #1976d2; border-radius: 6px; cursor: pointer;"
                                onclick="openClassDetail({{ $class['id'] }})">
            ...
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $s->current_enrollment ?? 0 }}/{{ $s->max_capacity }}</td>
                        <td class="text-end">
                            <button class="btn btn-sm btn-primary" onclick="openClassDetail({{ $s->id }})"><i class="fas fa-eye"></i></button>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
        ...
        <div class="table-responsive">
            <table class="table table-bordered mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="text-center align-middle fw-bold">Tuần {{ $wIndex+1 }}</td>
                        @foreach($week as $cell)
                        <td class="align-top" style="min-width: 180px;">
                            <div class="d-flex justify-content-between">
                                <span class="fw-semibold {{ $cell['inMonth'] ? '' : 'text-muted' }}">{{ $cell['day'] }}</span>
                            </div>
                            @foreach($cell['classes'] as $c)
                            <div class="border rounded p-2 mt-2" style="cursor:pointer; background:#f8fbff; border-left:3px solid #1976d2;" onclick="openClassDetail({{ $c['id'] }})">
