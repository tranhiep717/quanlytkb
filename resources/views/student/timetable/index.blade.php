        ...
        <div class="row g-3 align-items-end">
            ...
            <div class="col-12 col-lg-6 d-flex flex-wrap justify-content-lg-end align-items-center gap-2">
                <div class="btn-group" role="group" aria-label="View Mode">
                    <a class="btn btn-outline-secondary btn-sm {{ $view==='week' ? 'active' : '' }}" href="{{ route('student.timetable', ['academic_year'=>$academicYear,'term'=>$term,'view'=>'week','date'=>$baseDate]) }}">
                        <i class="fas fa-calendar-week me-1"></i>Tuần
                    </a>
                    <a class="btn btn-outline-secondary btn-sm {{ $view==='month' ? 'active' : '' }}" href="{{ route('student.timetable', ['academic_year'=>$academicYear,'term'=>$term,'view'=>'month','date'=>$baseDate]) }}">
                        <i class="fas fa-calendar-alt me-1"></i>Tháng
                    </a>
                    <a class="btn btn-outline-secondary btn-sm {{ $view==='list' ? 'active' : '' }}" href="{{ route('student.timetable', ['academic_year'=>$academicYear,'term'=>$term,'view'=>'list','date'=>$baseDate]) }}">
                        <i class="fas fa-list me-1"></i>Danh sách
                    </a>
                </div>

                <div class="btn-group" role="group" aria-label="Navigate">
                    <a class="btn btn-outline-secondary btn-sm" title="Trước" href="{{ route('student.timetable', ['academic_year'=>$academicYear,'term'=>$term,'view'=>$view,'date'=>$prevDate]) }}">
                    ...
                    <a class="btn btn-outline-secondary btn-sm" title="Sau" href="{{ route('student.timetable', ['academic_year'=>$academicYear,'term'=>$term,'view'=>$view,'date'=>$nextDate]) }}">
                        <i class="fas fa-chevron-right"></i>
                    </a>
                    <a class="btn btn-outline-primary btn-sm" href="{{ route('student.timetable', ['academic_year'=>$academicYear,'term'=>$term,'view'=>$view]) }}">Hôm nay</a>
                </div>
                    ...
                    @for($slot = 0; $slot < max(1, $maxSlots); $slot++)
                        ...
                        <td class="p-2" style="vertical-align: top;">
                            ...
                            <div class="class-box p-3" style="background: linear-gradient(135deg, #e3f2fd, #bbdefb); border-left: 4px solid #1976d2; border-radius: 6px; cursor: pointer;"
                                onclick="openStudentClassDetail({{ $class['id'] }})">
                                ...
                                @if(!empty($class['lecturer']))
                                <div class="mt-1">
                                    <small class="text-muted">
                                        <i class="fas fa-user me-1"></i>{{ $class['lecturer'] }}
                                    </small>
                                </div>
                                @endif
                            </div>
            ...
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $s->lecturer->name ?? ($s->lecturer_name ?? 'TBA') }}</td>
                        <td class="text-end">
                            <button class="btn btn-sm btn-primary" onclick="openStudentClassDetail({{ $s->id }})"><i class="fas fa-eye"></i></button>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
