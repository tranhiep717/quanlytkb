...
<div class="card mb-3">
    <div class="card-body">
        <div class="row g-3 align-items-end">
            <form class="col-12 col-lg-6" method="GET" action="{{ route('student.dashboard') }}">
                <div class="row g-2">
            ...
            <div class="col-12 col-lg-6 d-flex flex-wrap justify-content-lg-end align-items-center gap-2">
                <div class="btn-group" role="group" aria-label="View Mode">
                    <a class="btn btn-outline-secondary btn-sm {{ $view==='week' ? 'active' : '' }}" href="{{ route('student.dashboard', ['academic_year'=>$academicYear,'term'=>$term,'view'=>'week','date'=>$baseDate]) }}">
                        <i class="fas fa-calendar-week me-1"></i>Tuần
                    </a>
                    <a class="btn btn-outline-secondary btn-sm {{ $view==='month' ? 'active' : '' }}" href="{{ route('student.dashboard', ['academic_year'=>$academicYear,'term'=>$term,'view'=>'month','date'=>$baseDate]) }}">
                        <i class="fas fa-calendar-alt me-1"></i>Tháng
                    </a>
                    <a class="btn btn-outline-secondary btn-sm {{ $view==='list' ? 'active' : '' }}" href="{{ route('student.dashboard', ['academic_year'=>$academicYear,'term'=>$term,'view'=>'list','date'=>$baseDate]) }}">
                        <i class="fas fa-list me-1"></i>Danh sách
                    </a>
                </div>

                <div class="btn-group" role="group" aria-label="Navigate">
                    <a class="btn btn-outline-secondary btn-sm" title="Trước" href="{{ route('student.dashboard', ['academic_year'=>$academicYear,'term'=>$term,'view'=>$view,'date'=>$prevDate]) }}">
                    ...
                    <a class="btn btn-outline-secondary btn-sm" title="Sau" href="{{ route('student.dashboard', ['academic_year'=>$academicYear,'term'=>$term,'view'=>$view,'date'=>$nextDate]) }}">
                        <i class="fas fa-chevron-right"></i>
                    </a>
                    <a class="btn btn-outline-primary btn-sm" href="{{ route('student.dashboard', ['academic_year'=>$academicYear,'term'=>$term,'view'=>$view]) }}">Hôm nay</a>
                </div>

                <div class="btn-group" role="group" aria-label="Exports">
                    <a class="btn btn-outline-success btn-sm" href="{{ route('student.timetable.exportCsv', ['academic_year'=>$academicYear,'term'=>$term]) }}">
                        <i class="fas fa-file-excel me-1"></i>Excel/CSV
                    </a>
                    <a class="btn btn-outline-danger btn-sm" target="_blank" href="{{ route('student.timetable.print', ['academic_year'=>$academicYear,'term'=>$term]) }}">
                        <i class="fas fa-file-pdf me-1"></i>PDF
                    </a>
                    <a class="btn btn-outline-dark btn-sm" href="{{ route('student.exportIcs', ['academic_year'=>$academicYear,'term'=>$term]) }}">
                            ...
                            <div class="class-box p-3" style="background: linear-gradient(135deg, #e3f2fd, #bbdefb); border-left: 4px solid #1976d2; border-radius: 6px;">
                                ...
                                <div class="mt-2">
                                    <small class="text-muted" title="Giảng viên">
                                        <i class="fas fa-chalkboard-teacher me-1"></i>{{ $class['lecturer'] }}
                                    </small>
                                </div>
                                <div class="mt-1">
                                    <small class="text-muted">
            ...
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $s->lecturer->name ?? 'TBA' }}</td>
                        <td>{{ $s->course->credits ?? 0 }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
                    ...
                    <tr>
                        ...
                        <td class="align-top" style="min-width: 180px;">
                            ...
                            <div class="border rounded p-2 mt-2" style="background:#f8fbff; border-left:3px solid #1976d2;">
                                ...
                                <div class="small text-muted mt-1"><i class="fas fa-chalkboard-teacher me-1"></i>{{ $c['lecturer'] }}</div>
                            </div>
                            @endforeach
                        </td>
                        @endforeach
