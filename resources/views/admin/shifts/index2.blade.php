    ...
    <form method="GET" action="{{ route('shifts.index') }}" style="margin-bottom:20px;">
        <div style="display:flex; gap:12px; align-items:end;">
            <div style="flex:2;">
                ...
                <input type="text" name="q" value="{{ $filters['q'] ?? '' }}" placeholder="Mã ca / Tên ca"
            ...
            <a href="{{ route('shifts.index') }}" style="padding:10px 20px; border:1px solid #cbd5e0; border-radius:6px; text-decoration:none; color:#475569; font-weight:500;">Xóa bộ lọc</a>
            @endif
        </div>
    </form>
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px;">
                    ...
                    <div>
                        ...
                        <select name="day_of_week" required style="width:100%; padding:10px; border:1px solid #cbd5e0; border-radius:6px; font-size:14px;">
                            @for($i=1;$i<=7;$i++)
                                <option value="{{ $i }}" {{ old('day_of_week')==$i? 'selected':'' }}>{{ [1=>'Thứ 2',2=>'Thứ 3',3=>'Thứ 4',4=>'Thứ 5',5=>'Thứ 6',6=>'Thứ 7',7=>'CN'][$i] }}</option>
                                @endfor
                        </select>
                    </div>
                    <div>
